...
struct LikeService {
    ...
    static func getLikeList(completion: @escaping (NSMutableArray) -> Void) {
        ...
        UserService.posts(for: JUser.current) { (posts) in
           // post.isLiked
            for post in posts{
                dispatchGroup.enter()
                let likesRef = Database.database().reference().child("postLikes").child(post.key!)
                likesRef.observeSingleEvent(of: .value, with: { (snapshot) in
                    ...
                    if(snapshot.children.allObjects.count > 0)
                    {
                        ...
                        print(value)
                        let likes0 =  LikeList(postKey: snapshot.key, list: value)
                        array.add(likes0)
                    }
