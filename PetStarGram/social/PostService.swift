    ...
    static func create(for image: UIImage, thumb:UIImage, ratioMode:Int, completion: @escaping (Bool?) -> Void){
        ...
        StorageService.uploadImage(image, at: imageRef) { (downloadURL) in
            ...
            StorageService.uploadImage(thumb, at: thumbRef) { (downloadURL) in
                ...
                create(forURLString: urlString, thumb: thumbString,ratioMode:ratioMode, completion:completion)
            }
        }
    ...
    static func createVideo(for videoURL: URL, thumb:UIImage, ratioMode:Int, completion: @escaping (Bool?) -> Void) {
        ...
        StorageService.upload(videoURL, at: imageRef) { (downloadURL) in
            ...
            StorageService.uploadImage(thumb, at: thumbRef) { (downloadURL) in
                ...
                create(forURLString: urlString, thumb: thumbString, mediaType: 1, ratioMode:ratioMode, completion:completion)
            }
        }
 ...
 */
    ...
    static func delete(_ post:JPost, completion: @escaping (Bool) -> Void)
    {
        ...
        let userRef =     Database.database().reference().child("user-posts").child(post.poster.uid).child(post.key!)
        
        let timelineRef =     Database.database().reference().child("timeline").child(post.poster.uid).child(post.key!)
        let followingRef =     Database.database().reference().child("followers").child(post.poster.uid)
      ...
      //  let name = post.imageURL
        ...
        let reference  = Storage.storage().reference().child("images/posts/\(post.poster.uid)/\(name)")
      
        let thumb_reference  = Storage.storage().reference().child("images/posts/\(post.poster.uid)/thumbs/\(thumb_name)")
